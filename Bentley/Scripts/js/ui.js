...
function replaceSelects() {
    for (var q = 0; q < selects.length; q++) {
        ...
        selects[q].onchange = function () {
            var t_opt = this.getElementsByTagName('option');
            for (var i = 0; i < t_opt.length; i++) {
                if (t_opt[i].selected) {
                    var t_box = document.getElementById('mySelectText' + this._number);
                    if (t_box) {
                        t_box.innerHTML = t_opt[i].innerHTML;
                    }
                }
            }
        }
...
function selectMe(selectFieldId, linkNo, selectNo) {
    ...
    //__doPostBack(selectFieldId, linkNo);

    if (linkNo == 0) return false;
...
function initCufon() {
    ...
    //    Cufon.replace('.sb-nav > ul > li > a', { fontFamily: 'Gill Sans MT', hover: true });
    //    Cufon.replace('.search-result .search-parameters h2 > a', { fontFamily: 'Gill Sans', hover: true });

    // static elements
    //    Cufon.replace('.main-content .tabs-holder h2', { fontFamily: 'Gill Sans Light' });
    //    Cufon.replace('.next-steps p', { fontFamily: 'Gill Sans MT' });
    //    Cufon.replace('.search .models ul label, .search-parameters .model-col .drop-down form label', { fontFamily: 'Gill Sans MT' });
    ...
    //    Cufon.replace('.search .distance .measuring-bar li label', { fontFamily: 'Gill Sans' });
    //    Cufon.replace('.search .options h2', { fontFamily: 'Gill Sans' });
    //    Cufon.replace('.search .options .distance .currency-bar h3', { fontFamily: 'Gill Sans' });
    //    Cufon.replace('.search .select-country-menu h2', { fontFamily: 'Gill Sans Light' });

    //Cufon.replace('.babel', { fontFamily: 'Gill Sans' });    
}